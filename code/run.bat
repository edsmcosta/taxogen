...
IF %FIRST_RUN%==1  (
  ...
  poetry run python "cluster-preprocess.py" "%corpusName%"

  echo "Start preprocess.py -- %time%"
  poetry run python "preprocess.py" "%corpusName%"
...
poetry run python "compress.py" "-root" "..\data\%corpusName%\%taxonName%" "-output" "..\data\%corpusName%\taxonomies\%taxonName%.txt"
